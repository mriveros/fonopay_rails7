class NotificarUsuario < ActionMailer::Base
  ...
  def test_email(user_id, adjuntos)
    ...
    if (@user)
      ...
      to = @user.email
      mail(:to => to, :subject => "Aviso FonoPago", :from => "smarthub.py@gmail.com") 
  ...
  def email(usuario, password)
    ...
    mail(to: @usuario.email, subject: "FonoPago: Nuevo Usuario")
  ...
  def email_recuperar_password(usuario, password)
    ...
    mail(to: @usuario.email, subject: "FonoPago: Recuperar Contraseña")
