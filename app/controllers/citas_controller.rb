...
before_action :require_usuario
  ...
  def guardar
    ...
    ActiveRecord::Base.transaction do
      ...
      @cita.profesional_id = params[:cita][:profesional_id]
      ...
      if @cita.save
        ...
        @cita_ok = true
        registro_cobro = RegistroCobro.new()
        registro_cobro.cita_id = @cita.id
        registro_cobro.estado_cobro_id =  PARAMETRO[:estado_cobro_pendiente]
        registro_cobro.save
  ...
  def actualizar
    ...
    if @valido
      ...
      @cita.fecha_cita = params[:fecha_cita]
      @cita.paciente_id = params[:paciente_id]
      @cita.profesional_id = params[:v_cita][:profesional_id]
  ...
  def eliminar
    ...
    if @valido
      ...
      if @cita.destroy
        ...
        registro_cobro = RegistroCobro.where('cita_id = ?', @cita_elim.id).first
        
        if registro_cobro.present?

          registro_cobro.destroy
